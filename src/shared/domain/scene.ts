import { sceneElementSchema, sceneSchema } from '@/shared/schemas/validators';
import { infer3DKind, infer3DKindFromSceneElements, type Project3DKind } from '@/shared/domain/projectKind';
...
function normalizeType(rawType: unknown, kind: Project3DKind): SceneElements[number]['type'] {
    ...
    if (key.includes('cone') || key.includes('taper') || key.includes('pyramid')) return 'cone';
    if (key.includes('torus') || key.includes('ring') || key.includes('donut')) return 'torus';
    if (key.includes('plane') || key.includes('flat') || key.includes('panel')) return 'plane';
    if (key.includes('dome') || key.includes('hemisphere') || key.includes('half-sphere')) return 'half-sphere';
...
function normalizeDimensions(
    ...
    switch (type) {
        ...
        case 'torus': {
            const radius = clampPositive(Math.max(x, z) / 2 || 20);
            const tube = clampPositive(Math.min(x, z) / 6 || 4);
            return [radius, tube, 0];
        }
        case 'plane': {
            return [clampPositive(x), 0, clampPositive(z || y || x)];
        }
        case 'rounded-box':
        case 'box':
        default:
...
function normalizeMaterial(input: unknown): SceneElements[number]['material'] | undefined {
    if (input === 'plastic' || input === 'metal' || input === 'glass' || input === 'rubber' || input === 'emissive') {
        return input;
    }
    return undefined;
}
...
export function sanitizeSceneElements(
    ...
    for (const el of candidates) {
        ...
        const opacity = typeof el.opacity === 'number' ? Math.min(1, Math.max(0, el.opacity)) : undefined;
        const emissiveColor = typeof el.emissiveColor === 'string' ? el.emissiveColor : undefined;
        const emissiveIntensity = typeof el.emissiveIntensity === 'number' ? el.emissiveIntensity : undefined;
        const texture = typeof el.texture === 'string' ? el.texture : undefined;
...
function elementVolume(el: SceneElements[number]): number {
    ...
    switch (el.type) {
        ...
        case 'cone':
            return Math.abs((1 / 3) * Math.PI * a * a * b);
        case 'sphere':
            return Math.abs((4 / 3) * Math.PI * a * a * a);
        case 'half-sphere':
            return Math.abs((2 / 3) * Math.PI * a * a * a);
        case 'capsule':
            // Cylinder plus two hemispheres.
            return Math.abs(Math.PI * a * a * b + (4 / 3) * Math.PI * a * a * a);
        case 'torus':
            return Math.abs(2 * Math.PI * Math.PI * a * b * b);
        case 'plane':
            return Math.abs(a * c) * 0.01; // Negligible volume for planes
        default:
            return Math.abs(a * b * c);
    }
}
