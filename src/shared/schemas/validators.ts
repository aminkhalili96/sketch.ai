...
export const analyzeRequestSchema = z.object({
    image: z.string().min(1, 'Image is required').max(10_000_000, 'Image too large (max ~7.5MB)'),
...
const typeNormalization = z.preprocess((val) => {
    ...
    if (lower === 'ring' || lower === 'donut') return 'torus';
    if (lower === 'dome') return 'half-sphere';
    if (lower === 'flat' || lower === 'panel' || lower === 'surface') return 'plane';
    return lower;
}, z.enum(['box', 'rounded-box', 'cylinder', 'sphere', 'capsule', 'cone', 'torus', 'plane', 'half-sphere']));
...
export const sceneElementSchema = z.object({
    ...
    material: z.enum(['plastic', 'metal', 'glass', 'rubber', 'emissive', 'flat']).optional(),
    ...
    emissiveIntensity: safeNumberSchema.optional(),
    // Assembly layer and grouping
    layer: z.enum(['shell', 'internal', 'pcb', 'detail', 'label']).optional(),
...
export const chatRequestSchema = z.object({
    ...
    projectContext: z.object({
        ...
        outputs: z.object({
            ...
            'assembly-spec': z.string().optional(),
            'render-png': z.string().optional(),
            'cad-step': z.string().optional(),
            'cad-stl': z.string().optional(),
            safety: z.string().optional(),
            sustainability: z.string().optional(),
            'cost-optimization': z.string().optional(),
            dfm: z.string().optional(),
            marketing: z.string().optional(),
            'patent-risk': z.string().optional(),
        }).optional(),
    }).optional(),
    model: z.string().optional(),
});
...
export const agentsRouteRequestSchema = z.object({
    message: z.string().min(1, 'Message is required'),
    projectContext: agentsProjectContextSchema.optional(),
    model: z.string().optional(),
});

export const agentsRouteResponseSchema = z.object({
    mode: z.enum(['chat', 'plan']),
    requestedOutputs: z.array(requestedOutputSchema).optional(),
    confidence: z.number().min(0).max(1).optional(),
    reason: z.string().optional(),
});

export const render3dRequestSchema = z.object({
    projectDescription: z.string().min(1, 'Project description is required'),
    analysisContext: analysisResultSchema.optional(),
    assemblySpec: z.string().optional(),
    renderMode: z.enum(['assembled', 'exploded']).optional(),
    model: z.string().optional(),
});
...
export type AgentsRouteRequest = z.infer<typeof agentsRouteRequestSchema>;
export type AgentsRouteResponse = z.infer<typeof agentsRouteResponseSchema>;
