...
import { planAssemblySpec, refineAssemblySpec } from './assemblyPlanner';
import { assemblySpecToScene, type AssemblySpec } from '@/backend/pipeline/assemblySpec';
...
export async function orchestrate3DGeneration(
    ...
    if (visionAnalysis.objectType === 'enclosure') {
        logs.push('Planning enclosure assembly spec...');
        let assemblySpec = await planAssemblySpec(visionAnalysis, description, model);
        logs.push(`Assembly spec planned: ${assemblySpec.components.length} components, ${assemblySpec.ports.length} ports`);

        let currentScene = assemblySpecToScene(assemblySpec);
        ...
        while (iteration < maxIterations) {
            ...
            logs.push('Critiquing scene from assembly spec...');
            ...
            logs.push('Refining assembly spec...');
            assemblySpec = await refineAssemblySpec(assemblySpec, critique, description, model);
            currentScene = assemblySpecToScene(assemblySpec);
        }

        const finalCritique = critique ?? await critiqueScene(visionAnalysis, currentScene, model);
        logs.push(`Final structural score: ${finalCritique.score}`);
        logs.push('Visual polish skipped for assembly-spec pipeline');
    ...
    // Final structural validation — reuse last critique if already acceptable
    const finalCritique = (critique?.isAcceptable && critique?.score >= minAcceptableScore)
        ? critique
        : await critiqueScene(visionAnalysis, currentScene, model);
    logs.push(`Final structural score: ${finalCritique.score}`);
    ...
    if (!skipVisualPolish) {
        ...
        // Final visual score — reuse last visual critique if already acceptable
        const finalVisualCritique = (visualCritique?.isAcceptable && visualCritique?.score >= minVisualScore)
            ? visualCritique
            : await critiqueVisualAppeal(
                { elements: currentScene },
                description,
                model
            );
