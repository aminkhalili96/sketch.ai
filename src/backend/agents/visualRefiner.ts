...
const VISUAL_REFINER_PROMPT = `You are a professional 3D product designer. Improve this scene's VISUAL APPEAL based on the critique while maintaining enterprise-grade detail.
...
1. **Color Improvements**
   ...
   - PCB boards MUST be green (#2D5016), not gray
   - IC chips MUST be dark (#1A1A1A or #222222), metallic finish
   - Batteries MUST be dark gray (#3A3A3A)
...
2. **Polish & Refinement**
   ...
   - Use appropriate materials: "plastic" for bodies, "metal" for accents/chips, "rubber" for grips

3. **Internal Component Detail**
   - Add small marking dots (tiny dark cylinder) on IC chip tops
   - Ensure connectors have metal material and realistic color (#B7BCC2)
   - Add solder pads as tiny metal cylinders on PCB surface if missing
   - LEDs should use "emissive" material with emissiveColor and emissiveIntensity (1.5-3.0)

4. **Proportion Fixes**
   - Adjust sizes for better visual balance
   - Ensure hierarchy (shell largest, PCB fits inside, chips on PCB)

5. **Professional Touches**
   - Add subtle details: indicator lights using material "emissive"
   - Use opacity (0-1) for glass panels, display covers, or see-through elements
   - Use cone shapes for tapered elements, torus for rings/bezels, half-sphere for domes/sensor covers
   - Use plane for flat screens, labels, or display surfaces
...
- PRESERVE all layer and group annotations — do NOT remove them
- Available materials: plastic, metal, glass, rubber, emissive
- Available per-element properties: opacity (0-1), emissiveColor, emissiveIntensity
...
Return JSON with the complete refined scene:
{
  ...
  "changesApplied": [
    ...
    "Fixed PCB color to standard green #2D5016"
  ],
  "summary": "Applied modern tech aesthetic with white body and blue accents. Fixed internal component colors."
}
