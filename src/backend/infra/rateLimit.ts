 ...
 *
 * Trust hierarchy: API key > x-vercel-forwarded-for > x-forwarded-for (first IP only) > x-real-ip > 'unknown'
 */
export function getClientIdentifier(request: Request): string {
    ...
    // Prefer Vercel's trusted header (set by the platform, not spoofable by clients)
    const vercelForwarded = request.headers.get('x-vercel-forwarded-for');
    if (vercelForwarded) {
        return `ip:${vercelForwarded.split(',')[0].trim()}`;
    }

    // Fall back to standard forwarded header (first IP only to avoid spoofing via appended IPs)
