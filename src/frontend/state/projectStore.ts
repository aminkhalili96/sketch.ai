...
import type { Project, Message, AnalysisResult, ProjectOutputs, ProjectMetadata } from '@/shared/types';
import { DEFAULT_OPENAI_TEXT_MODEL, normalizeModelId } from '@/shared/ai/modelCatalog';
...
interface ProjectState {
    ...
    setDemoPreset: (presetId: string | null) => void;
            ...
            setDemoPreset: (presetId) => {
                ...
                if (project) {
                    set({
                        currentProject: {
                            ...project,
                            demoPresetId: presetId || undefined,
                            updatedAt: new Date(),
                        },
                    });
                }
...
// Granular selectors — subscribe to specific slices to avoid unnecessary re-renders
export const useProjectDescription = () => useProjectStore((s) => s.currentProject?.description ?? '');
export const useProjectOutputs = () => useProjectStore((s) => s.currentProject?.outputs ?? {});
export const useProjectAnalysis = () => useProjectStore((s) => s.currentProject?.analysis);
export const useIsGenerating = () => useProjectStore((s) => s.isGenerating);
export const useIsAnalyzing = () => useProjectStore((s) => s.isAnalyzing);
export const useIsExporting = () => useProjectStore((s) => s.isExporting);
export const useSelectedModel = () => useProjectStore((s) => s.selectedModel);
