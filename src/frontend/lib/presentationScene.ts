import { computeSceneBounds, parseSceneElements } from '@/shared/domain/scene';
...
export type PresentationElement = {
    type: 'box' | 'rounded-box' | 'cylinder' | 'sphere' | 'capsule' | 'torus';
...
export function buildPresentationScene(options: {
    ...
    const demoPreset = getDemoPreset(options.demoPresetId);
    if (demoPreset) {
        return demoPreset.scene as PresentationScene;
    }
